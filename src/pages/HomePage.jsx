...
const notifyCreated = () => {
  toast("New note added", {
    ...
    removeDelay: 1000,
    className:
      "flex items-center max-w-xs p-1 mb-2 md:p-2 md:mb-3 text-gray-500 bg-gray-300 rounded-lg shadow-sm dark:text-gray-400 dark:bg-gray-800",
  });
};
      ...
      const userNotes = await Promise.all(
        response.documents.map(async (note) => {
          ...
          const decryptedTitle = note.title
            ? await decryptNote(note.title)
            : "";
          return { ...note, content: decryptedContent, title: decryptedTitle };
        })
      );
      ...
      <main className="p-8">
        <div className="flex justify-between items-center mb-8">
          ...
          <button
            ...
            className="px-1.5 py-0.5 md:px-4 md:py-2 font-semibold text-white bg-blue-600 rounded-md hover:bg-blue-700"
          >
            + New Note
          </button>
        </div>
